# spec/level3_spec.rb
require 'json'
require_relative '../../level3/level3'
...
RSpec.describe Level3 do
  ...
  let(:rental_3) { build_rental_payload( id: 3, car_id: 1, start_date: "2015-07-03", end_date: "2015-07-14", distance: 1000) }
  ...
  context "with file-based data" do
    let(:file_input) do
      File.read(File.join(__dir__, '../../level3/data', 'input.json'))
    end

    let(:expected_output_file) do
      File.read(File.join(__dir__, '../../level3/data', 'expected_output.json'))
    end
