# spec/level4_spec.rb
require 'json'
require_relative '../../level4/level4'
...
RSpec.describe Level4 do
  ...
  subject(:level4) { described_class.new(input_data) }
  ...
  context "with file-based data" do
    let(:file_input) do
      File.read(File.join(__dir__, '../../level4/data', 'input.json'))
    end

    let(:expected_output_file) do
      File.read(File.join(__dir__, '../../level4/data', 'expected_output.json'))
    end
